<template>
  <div class="menuBody"
       style="bottom: 0px;padding: 1px;">

    <template v-if="!ifshowBetInfo">
      <div id='game'>
        <div class="sublist">
          <div class="menu">
            <button v-for="(item,index) in bocaiTypeList"
                    class="layui-btn layui-btn-mini layui-btn-radius"
                    :class="['OddsCategory'+item.bocaiTypeId,item.bocaiTypeId == bocaiTypeIdSele? 'selected':'']"
                    @click="hisOrderSele(item.bocaiTypeId)">{{item.bocaiTypeName}}</button>
          </div>
        </div>
      </div>
      ...
      <table border="0"
        ...
        <tbody id="orderList">
          ...
          <tr v-else
              v-for="item in nowWeekPage">
            <td>{{item.createDateStr}}</td>
            <td style="text-align:right">{{item.betsMoneySum}}</td>
            <td style="text-align:right">
              <font color="#00F"
                    title="历史详情"
                    @click="goBetInfo(item.createDateStr)">{{item.winnerMoneySum}}</font>
            </td>
    ...
    <template v-else>
      <div style="width:100%;text-align:center;">
        <a @click="backBetHis"
           style="width:80%;margin-bottom:10px;margin-top:10px"
           class="layui-btn layui-btn-small layui-btn-normal">返回上层</a>
      </div>
      ...
      <ul class="list">
        <li v-for="(item,index) in betInfo.list">
          <div class="card">
            ...
            <div>游戏类型：<font color="green">{{item.bocaiTypeName}}</font>
            </div>
            <div>投注内容：<font color="blur">{{item.bocaiCategory2Name}}</font> {{item.bocaiOddName}}@<font color="red">{{item.bocaiOdds}}</font>
            </div>
            <div>投注金额：<font color="red">{{item.betsMoney}}</font>&nbsp;&nbsp;&nbsp;&nbsp;可赢：<font color="red">{{(+item.winnerMoney-item.betsMoney*1).toFixed(2)}}</font>
            </div>
            <div>退水：<font color="green">{{item.userDewater}}</font>&nbsp;&nbsp;&nbsp;&nbsp;派彩：<font color="red">{{(+item.paicai+item.userDewater).toFixed(2)}}</font>
            </div>
      ...
      <div style="width:100%;text-align:center;">
        <a @click="getbetInfo"
           style="width:80%;margin-bottom:10px;margin-top:10px"
           class="layui-btn layui-btn-small layui-btn-normal">点击加载更多</a>
      </div>
      ...
      <footer style="">
        <div style="width:100%;text-align:center;color:#fff">
          合计: &nbsp;&nbsp;
          投注额: <span class="qs"
                id="order_money">{{sumData.betsMoneySum.toFixed(2)}}</span> &nbsp;&nbsp;
          可赢金额：<span class="qs"
                id="order_money">{{sumData.winnerMoneySum.toFixed(2)}}</span> &nbsp;&nbsp;
          派彩：<span class="qs"
                id="order_win">{{sumData.winnerMoneyResultSum.toFixed(2)}}</span>
        </div>
      </footer>
